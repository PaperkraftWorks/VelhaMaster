...
class VelhaG(object):
    ...
    def __init__(self,player):
        ...
        for c in range(3):
            for a in range(3):
                self.winin[c*3+a+1]=\
                [[3*9*c+(9*b+z)+3*a for z in range(3)] for b in range(3)],\
                [[3*9*c+9*b+z+3*a for b in range(3)] for z in range(3)],\
                [[3*9*c+(9*b)+b+3*a for b in range(3)],[3*9*c+9*b+2-b+3*a for b in range(3)]]
    ...
    def show(self):
        ...
        for n in range(9):
            ...
            if (n+1)%3==0:
                ...
                print("  +",*['+' if (j==5)or(j==11) else "-" for j in range(17)], "+")
    def positions(self,player):
        self.pos={}
        self.pos[player]=[ index for  index,value in enumerate(self.velhag) if value == player]
    ...
    def fullwin(self,index):
        for a in range(3):
            for b in range(3):
                self.mover(self.player,3*int(index/3)-2+a if index%3==0 else 3*int(index/3)+1+a,1+b if index%3==1 else 4+b if index%3==2 else  7+b)
